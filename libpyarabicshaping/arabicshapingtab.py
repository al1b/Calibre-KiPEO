...
# generated by gen-arabic-shaping-tab (GNU FriBidi 0.19.2)
...
def FRIBIDI_ACCESS_SHAPE_TABLE(table,minx,maxx,x,shape):
	if x < minx or x > maxx:
		return x
	s = table[x-minx][shape]
	if s == x and shape > 2:
		s = table[x-minx][shape - 2]
	return s

def FRIBIDI_GET_ARABIC_SHAPE_NSM(x,shape):
	return FRIBIDI_ACCESS_SHAPE_TABLE(NSMShap, 0x064B, 0x0652, x, shape)

def FRIBIDI_GET_ARABIC_SHAPE_PRES(x,shape):
	return FRIBIDI_ACCESS_SHAPE_TABLE(ArShap, 0x0621, 0x06d3, x, shape)
